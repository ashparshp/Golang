...
func (m *DBModel) Authenticate(email, password string) (int, error) {
	...
	row := m.DB.QueryRowContext(ctx, `
		select id, password from users where email = ?`, strings.ToLower(email))
...
// GetAllOrdersPaginated retrieves all orders from the database with pagination
func (m *DBModel) GetAllOrdersPaginated(page, pageSize int) ([]Order, int, int, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	...
	var totalCount int
	err = m.DB.QueryRowContext(ctx, query).Scan(&totalCount)
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, 0, 0, nil
		}
	...
	return orders, lastPage, totalCount, nil
}
...
// GetAllSubscriptionsPaginated retrieves all subscriptions from the database with pagination
...
// GetSaleDetails retrieves the details of a sale by its ID
func (m *DBModel) GetSaleDetails(id int) (Order, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
